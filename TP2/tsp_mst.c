...
double tsp_mst(point *V, int n, int *P, graph T) {
  ...
  edge *E = malloc(sizeof(edge)*(n*(n-1))/2); // à remplacer par un malloc(...)
  int indice = 0;
  for(int i = 0; i<n-1;i++){
    for(int j = i+1; j<n ; j++){
      E[indice].u=i;
  ...
  int cpt = 0;
  for(int i = 0; cpt<n-1;i++){
    if(Find(E[i].u,parent)!=Find(E[i].v,parent)) {
    addEdge(T,E[i].u,E[i].v);
    Union(Find(E[i].u,parent),Find(E[i].v,parent),parent,rank);
    cpt++;
    }
  }
  ...
  if(T.deg[0]<0) return 0; // si pas d'arbre, alors pas de tournée
