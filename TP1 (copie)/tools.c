...
static int getClosestVertex(double x, double y) {
  ...
  int res = 0;
  double dmin = (x - POINT[0].x) * (x - POINT[0].x) +
...
static int const NCOLOR = (int)(sizeof(color)/sizeof(*color));
...
static void makeImage(grid *G) {
  ...
  bool const fin =
    (G->mark[G->start.x][G->start.y] == M_PATH) &&
    ...
    for (int i = 0; i < G->X; i++) {
      ...
      do { // do...while(0) pour permettre des break
        ...
        if (m == M_USED || m == M_USED2) {
          ...
          position p = {.x = i, .y = j};
          t1 = (m == M_USED) ? distL2(G->start, p) / dmax : distL2(G->end, p) / dmax;
  ...
  for (int t = 0; t < m; t++) // répète m fois
    for (int i = 1; i < G.X - 1; i++)
      for (int j = 1; j < G.Y - 1; j++) {
        int c = 0; // c = nombre de voisins de (i,j) de valeur "type"
        for (int k = 0; k < 8; k++)
...
void centering(double margin){
  ...
  point const M = (point){ (Pmin.x+Pmax.x)/2, (Pmin.y+Pmax.y)/2 }; // centre points
...
void drawX(point *V, int n, int *P, int k, graph *G) {
  ...
  if (V && P && (P[0]>=0) && ((G && (mst&2)) || (!G && (mst&1)))) {
    ...
    else glColor3f(CLR_PATH);
    for (int i = 0; i < k - 1; i++){
      if(oriented) drawEdge(V[P[i]], V[P[(i + 1) % n]]);
      else         drawLine(V[P[i]], V[P[(i + 1) % n]]);
    }
    if (root) {
      glColor3f(CLR_ROOT);
      if(oriented && n>0) drawEdge(V[P[0]], V[P[1%n]]);
      else                drawLine(V[P[0]], V[P[1%n]]);
    }
  }
...
void waitGridDelay(grid G, unsigned int delay, unsigned int frame_delay) {
  unsigned int const last_tick = SDL_GetTicks();
...
void drawGrid(grid G) {
  ...
  unsigned int const frame_rate = 50; // 50 fps
  unsigned int const frame_delay = 1000 / frame_rate; // durée en millisecondes

  call_count++;

  unsigned int const current_tick = SDL_GetTicks();
  ...
  do {
      ...
      if (e.button.button == SDL_BUTTON_LEFT) {
        ...
        if (hover) {
          ...
          if ((x - POINT[v].x)*(x - POINT[v].x) +
              (y - POINT[v].y)*(y - POINT[v].y) < (sizePt*sizePt)+2 )
            selectedVertex = v;
        }
        mouse_ldown = true;
      }
    ...
    case SDL_MOUSEMOTION:
      if (hover && !mouse_rdown && mouse_ldown && selectedVertex >= 0) {
        pixelToCoord(e.motion.x, e.motion.y,
                     &(POINT[selectedVertex].x),
