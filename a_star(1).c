...
int fcmp_node(const void *x, const void *y) {
  return ((node)x)->score - ((node)y)->score  <= 0 ? -1 : 1 ;
}

int sum_deg(grid G) {
  //return G.X * G.Y;
  return ((G.X * G.Y)*((G.X * G.Y)-1)) /2;
}

int comp_pos(position p1, position p2) {
    // printf("%d-%d vs %d-%d \n", p1.x, p1.y, p2.x, p2.y);
        
  return p1.x == p2.x && p1.y == p2.y ? 0 : -1;
}

node create_node(int x, int y, int cost, int score, node parent) {
...
double A_star(grid G, heuristic h){
  ...
  /*struct node n = { { 5, 5 }, 10, 15 , NULL}; //add create node
  struct node m = { { 4, 4 }, 5, 20, NULL };*/

 // printf("%d\n", fcmp_node(&m, &n));

  heap tas = heap_create(sum_deg(G), &fcmp_node);

  position start = { G.start.x, G.start.y };
  position end = { G.end.x, G.end.y };


  //  printf("%d et %d\n", start.x, start.y);
  node nodeStart = create_node(start.x, start.y,0, 0 + h(start,end,&G) , NULL);
  heap_add(tas, nodeStart);

  while(!heap_empty(tas)){

    node u = heap_pop(tas);

    if(comp_pos(u->pos, end) == 0){
        double c = u->cost;
        while(u != NULL ){
   ...
   // printf("%d et %d\n", u->pos.x, u->pos.y);
    if(G.mark[u->pos.x][u->pos.y] == M_USED)continue;
    ...
    for(int x=u->pos.x-1; x<=u->pos.x+1; x++){
      for(int y=u->pos.y-1; y<=u->pos.y+1; y++){
        ...
        if(( x != u->pos.x || y != u->pos.y )&&  G.mark[x][y] != M_USED ){
          position vpos = { x, y };
          int c = u->cost + weight[G.value[x][y]];

          node n = create_node(vpos.x, vpos.y, c, c + h(vpos,end,&G) , u);
          G.mark[x][y] = M_FRONT; 
          heap_add(tas, n);
    ...
    /* if(u->pos.x == end.x && u->pos.y == end.y){
        return 1;
    }*/
