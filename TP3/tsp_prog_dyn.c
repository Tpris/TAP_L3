...
double tsp_prog_dyn(point *V, int n, int *Q) {
  ...
  */
  ...
  for (int S=1; S<C; S++){
    for (int t=0; t<L; t++){
      //verif case bleu ou pas
      int T = DeleteSet(S,t);
      if(T!=S){
        if(!T){// taille 0 pour E privé de t si card = 1
          D[t][S].length = dist(V[n-1], V[t]);
          D[t][S].pred = n-1;
        }
        else{
          D[t][S].length = DBL_MAX;
          for (int x=0; x<L; x++){
            if(DeleteSet(T,x)==T) continue;
            // ici x est dans T
            int d = D[x][T].length + dist(V[x], V[t]);
            if(d<D[t][S].length){
              D[t][S].length= d;
              D[t][S].pred = x;
            }
  ...
  if (running) {
    ...
    int k = ExtractPath(D, ind, C-1, n, Q); // extrait Q depuis D[ind][C-1]
  }
