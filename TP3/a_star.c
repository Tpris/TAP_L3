...
int fcmp_node(const void *x, const void *y) {
  ...
  return (a<=b)? -1 : 1; // ou encore return (a>b) - (a<b);
}

node create_node(position p, double cost, double score, node parent){
...
double A_star(grid G, heuristic h){
  ...
  heap Q = heap_create(8*G.X*G.Y,&fcmp_node);
  int cost = 0;
  int score = cost + h(G.start,G.end,&G);

  node s = create_node(G.start,cost, score, NULL);
  ...
  while(!heap_empty(Q)){
    ...
    for(int j =-1; j<=1; j++){
      for(int i =-1; i<=1; i++){
        if(i==0 && j==0) continue;
        position p = (position){u->pos.x+i, u->pos.y+j};
        if(G.value[p.x][p.y]== V_WALL) continue;
        if(G.mark[p.x][p.y]== M_USED) continue;
        double c = u->cost + weight[G.value[p.x][p.y]];
        double sc = c + h(p,G.end,&G);
        node v = create_node(p, c, sc, u);
        heap_add(Q, v);
